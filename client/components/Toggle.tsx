...
export const Toggle: React.FC<ToggleProps> = ({ checked, onChange, label, id }) => {
    return (
        <div className="flex items-center gap-2 cursor-pointer group" onClick={() => onChange(!checked)}>
            ...
            <label htmlFor={id} className="text-[10px] font-extrabold text-slate-500 uppercase tracking-widest cursor-pointer select-none group-hover:text-slate-700 transition-colors">
                {label}
            </label>
        </div>
    );
